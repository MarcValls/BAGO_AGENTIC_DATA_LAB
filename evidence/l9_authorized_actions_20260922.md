...
segunda acción queda `EXECUTED` en la rama de trabajo y respaldada por la
suite completa; no altera la veracidad histórica del bloqueo anterior.

## Límite de validación

La propuesta `file_creator` sigue siendo `REQUIRE_HUMAN` para cualquier efecto
material, aunque su contenido ahora es un test válido del contrato. Esta
ejecución no convierte L9 completo en `VALIDATED`: AWS Bedrock real,
commercetools real, MCP externo y la validación de vídeo siguen fuera de esta
ejecución.
